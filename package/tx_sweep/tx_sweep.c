/*
 * tx_sweep - Driver utility for transmitting a frequency sweep pattern
 *
 * Derived from:
 *
...
bool cfg_ad9361_streaming_ch(struct iio_context *ctx, struct stream_cfg *cfg, enum iodev type, int chid)
{
	...
	wr_ch_d(chn, "hardwaregain", cfg->gain_db);
...
void usage()
{
	fprintf(stderr, "Usage: tx_sweep [-f LO_FREQ] [-r RATE] [-g GAIN] [IIO_CONTEXT]\n\n");
	fprintf(stderr, "\tLO_FREQ\tmiddle frequency of the sweep in MHz (default: 433)\n\n");
	fprintf(stderr, "\tGAIN\toutput gain in dB (default: -80)\n\n");
	fprintf(stderr, "\tRATE\tsweep rate per second (default: 40)\n\n");
	fprintf(stderr, "\tIIO_CONTEXT\tdo not worry about this one\n\n");
}
...
int main (int argc, char **argv)
{
	...
	// TX stream config
	txcfg.bw_hz = MHZ(50); // 50 MHz bandwidth
	txcfg.fs_hz = MHZ(61.440);   // 2.5 MS/s tx sample rate
	txcfg.lo_hz = MHZ(433); // 2.5 GHz rf frequency
	txcfg.gain_db = -80; // 2.5 GHz rf frequency
	txcfg.rfport = "A"; // port A (select for rf freq.)

	int opt;
	while ((opt = getopt(argc, argv, "f:r:g:")) != -1) {
		switch (opt){
			case 'f':
				txcfg.lo_hz = MHZ(atof(optarg));
				break;
			case 'r':
				rate = (int) (atof(optarg)*70);
				break;
			case 'g':
				txcfg.gain_db = (int) (atof(optarg));
				break;
			default:
				usage();
				exit(1);
...

	{
		...
		mem = mmap(NULL, 0x4000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0x79024000);
		IIO_ENSURE(mem != MAP_FAILED);
	}
	mem[0xbc/4] = (rate << 16) | (0x8000) | (mem[0xbc/4] & 0x7fff);
	...
	if (optind >= argc) {
		...
		IIO_ENSURE((ctx = iio_create_context_from_uri(argv[optind])) && "No context");
	}
